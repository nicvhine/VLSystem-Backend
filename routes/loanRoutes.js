...
router.get("/details/:loanId", async (req, res) => {
  ...
  try {
    ...
    const result = {
      ...
      totalPayable: loan.totalPayable || application?.totalPayable || 0,
      monthlyDue: loan.monthlyDue || application?.appMonthlyDue || 0,
      termsInMonths: loan.termsInMonths || application?.appLoanTerms || 0,
      interestRate: loan.interestRate || application?.appInterest || 0,
      dateDisbursed: loan.dateDisbursed,
      status: loan.status,
      balance: loan.balance,
      paidAmount: loan.paidAmount,
...
router.get('/:loanId', async (req, res) => {
  ...
  try {
    ...
    // Map current loan to match frontend expectations
    const currentLoan = isActive ? {
      ...
      remainingBalance: loan.balance || loan.totalPayable,
      dateDisbursed: loan.dateDisbursed,
      status: loan.status,
    } : undefined;
    ...
    res.json({
      ...
      numberOfLoans: application.numberOfLoans || 1,
      characterReferences: application.appReferences || [],
      profilePic: loan.profilePic || application.profilePic || null,
