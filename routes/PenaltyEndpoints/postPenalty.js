...
module.exports = (db) => {
  ...
  router.post("/endorse", authenticateToken, authorizeRole("collector"), async (req, res) => {
    try {
      const { referenceNumber, reason, fileUrl } = req.body;
      const userId = req.user.id;

      const collection = await db.collection("collections").findOne({ referenceNumber });
      if (!collection) return res.status(404).json({ message: "Collection not found" });

      const result = await service.endorsePenalty(collection, { reason, fileUrl }, userId);
