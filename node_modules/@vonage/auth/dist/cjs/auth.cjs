...
class Auth {
    ...
    constructor(opts) {
        ...
        if ((0, node_fs_1.existsSync)(opts?.privateKey)) {
            log('Reading private key file');
            privateKey = (0, node_fs_1.readFileSync)(opts?.privateKey).toString();
        }
    ...
    getQueryParams = async (params) => {
        if (!this.apiKey) {
            throw new index_js_2.MissingApiKeyError();
        }
        if (!this.apiSecret) {
            throw new index_js_2.MissingApiSecretError();
        }
        if (typeof this.apiKey !== 'string') {
            throw new index_js_2.InvalidApiKeyError();
        }
        if (typeof this.apiSecret !== 'string') {
            throw new index_js_2.InvalidApiSecretError();
        }
        return {
            ...params,
            api_key: this.apiKey,
    ...
    createSignatureHash = async (params) => {
        ...
        if (!this.signature?.algorithm) {
            throw new index_js_2.MissingSignatureError();
        }
        ...
        switch (this.signature.algorithm) {
            case index_js_1.AlgorithmTypes.md5hash:
                returnParams.sig = (0, node_crypto_1.createHash)('md5')
                    .update(`${stringifiedParamsforSigning}${this.signature.secret}`)
                    .digest('hex');
                break;
            case index_js_1.AlgorithmTypes.md5hmac:
                returnParams.sig = (0, node_crypto_1.createHmac)('md5', this.signature.secret)
                    .update(stringifiedParamsforSigning)
                    .digest('hex');
                break;
            case index_js_1.AlgorithmTypes.sha1hmac:
                returnParams.sig = (0, node_crypto_1.createHmac)('sha1', this.signature.secret)
                    .update(stringifiedParamsforSigning)
                    .digest('hex');
                break;
            case index_js_1.AlgorithmTypes.sha256hmac:
                returnParams.sig = (0, node_crypto_1.createHmac)('sha256', this.signature.secret)
                    .update(stringifiedParamsforSigning)
                    .digest('hex');
                break;
            case index_js_1.AlgorithmTypes.sha512hmac:
                returnParams.sig = (0, node_crypto_1.createHmac)('sha512', this.signature.secret)
                    .update(stringifiedParamsforSigning)
                    .digest('hex');
                break;
            default:
                throw new index_js_2.InvalidSignatureAlgorithmError();
        }
        return returnParams;
    };
}
