...
export class Verify2 extends Client {
    ...
    async getTemplatePage(params = {}) {
        ...
        const templates = (resp.data._embedded?.templates || []).map((template) => Client.transformers.camelCaseObjectKeys(template));
        return {
    ...
    async getTemplateFragmentPage(params) {
        ...
        const fragments = (resp.data._embedded?.template_fragments || []).map((fragment) => Client.transformers.camelCaseObjectKeys(fragment));
        return {
            pageSize: resp.data.page_size,
            page: resp.data.page,
            totalPages: resp.data.total_pages,
