...
class SMS extends server_client_1.Client {
    ...
    async send(params) {
        ...
        const failures = messages.reduce((failures, { status }) => status !== index_js_2.SMSStatus.SUCCESS ? failures + 1 : failures, 0);
        if (failures < 1) {
            return messageData;
        }
        if (failures === totalMessages) {
            throw new index_js_1.MessageSendAllFailure(messageData);
        }
        throw new index_js_1.MessageSendPartialFailure(messageData);
    }
    /**
