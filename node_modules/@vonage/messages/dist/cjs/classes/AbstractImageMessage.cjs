"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AbstractImageMessage = void 0;
const AbstractMessage_js_1 = require("./AbstractMessage.js");
const MessageTypes_js_1 = require("../enums/MessageTypes.js");
/**
 * An abstract base class for image messages.
 */
class AbstractImageMessage extends AbstractMessage_js_1.AbstractMessage {
    /**
     * The type of message (always 'image').
     */
    messageType = MessageTypes_js_1.MessageTypes.IMAGE;
    image;
    /**
     * Constructs a new `AbstractImageMessage` instance.
     *
     * @param {MessageParamsImage} params - The parameters for creating an image message.
     */
    constructor(params) {
        super(params);
        this.image = params.image;
    }
}
exports.AbstractImageMessage = AbstractImageMessage;
//# sourceMappingURL=AbstractImageMessage.js.map