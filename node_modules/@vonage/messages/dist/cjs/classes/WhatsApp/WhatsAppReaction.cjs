"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.WhatsAppReaction = void 0;
const AbstractMessage_js_1 = require("../AbstractMessage.js");
const index_js_1 = require("../../enums/index.js");
/**
 * Represents a reaction message for WhatsApp.
 *
 * @group WhatsApp
 */
class WhatsAppReaction extends AbstractMessage_js_1.AbstractMessage {
    /**
     * The channel for this message (always 'whatsapp').
     */
    channel = index_js_1.Channels.WHATSAPP;
    /**
     * The type of message (always 'reaction').
     */
    messageType = index_js_1.MessageTypes.REACTION;
    /**
     * The reaction to send
     */
    reaction;
    /**
     * The WhatsApp Context
     */
    context;
    /**
     * Sends a reaction message to a WhatsApp user.
     *
     * @param {WhatsAppReactionParams} params - The parameters for creating a WhatsApp reaction message.
     * @example
     * Send a reaction
     * ```ts
     * import { WhatsAppReaction } from '@vonage/messages';
     *
     * const { messageUUID } = await messagesClient.send(new WhatsAppReaction({
     *  to: TO_NUMBER,
     *  from: FROM_NUMBER,
     *  reaction: {
     *    action: 'react',
     *    emoji: '😍',
     *  }
     *  clientRef: 'my-personal-reference',
     * }));
     *
     * console.log(`Message sent successfully with UUID ${messageUUID}`);
     * ```
     *
     * @example
     * Remove reaction
     * ```ts
     * import { WhatsAppReaction } from '@vonage/messages';
     *
     * const { messageUUID } = await messagesClient.send(new WhatsAppReaction({
     *  to: TO_NUMBER,
     *  from: FROM_NUMBER,
     *  reaction: {
     *    action: 'unreact',
     *  }
     *  clientRef: 'my-personal-reference',
     * }));
     *
     * console.log(`Message sent successfully with UUID ${messageUUID}`);
     * ```
     */
    constructor(params) {
        super(params);
        this.reaction = params.reaction;
        if (params.context) {
            this.context = params.context;
        }
    }
}
exports.WhatsAppReaction = WhatsAppReaction;
//# sourceMappingURL=WhatsAppReaction.js.map