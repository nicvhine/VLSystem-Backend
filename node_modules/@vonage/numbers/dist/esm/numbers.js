...
export class Numbers extends Client {
    ...
    async buyNumber(params) {
        const resp = await this.sendFormSubmitRequest(`${this.config.restHost}/number/buy`, Client.transformers.snakeCaseObjectKeys(params));
        return {
    ...
    async cancelNumber(params) {
        const resp = await this.sendFormSubmitRequest(`${this.config.restHost}/number/cancel`, Client.transformers.snakeCaseObjectKeys(params));
        return {
            errorCode: `${resp.data['error-code']}`,
            errorCodeLabel: resp.data['error-code-label'],
        };
    ...
    async getAvailableNumbers(filter) {
        omit(Client.transformers.snakeCaseObjectKeys({
            ...filter,
            ...buildSearch(filter),
    ...
    async getOwnedNumbers(filter) {
        ...
        const resp = await this.sendGetRequest(`${this.config.restHost}/account/numbers`, Client.transformers.snakeCaseObjectKeys(filter));
        return resp.data;
    }
    /**
