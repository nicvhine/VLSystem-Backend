...
const buildSearch = ({ endsWith, startsWith, contains, searchPattern, pattern, country, }) => {
    searchPattern = searchPattern ?? index_js_1.SearchPattern.CONTAINS;
    if (pattern) {
        return {
    ...
    if (contains) {
        return {
            search_pattern: index_js_1.SearchPattern.CONTAINS,
            pattern: contains,
            country: country,
        };
    }
    if (endsWith) {
        return {
            search_pattern: index_js_1.SearchPattern.ENDS_WITH,
            pattern: endsWith,
            country: country,
        };
    }
    if (startsWith) {
        return {
            search_pattern: index_js_1.SearchPattern.START_WITH,
            pattern: startsWith,
            country: country,
        };
    }
...
const sortFeatures = (features) => {
    ...
    if (features.length === 3) {
        return [index_js_1.Feature.SMS, index_js_1.Feature.MMS, index_js_1.Feature.VOICE].join(',');
    }
    const newOrder = [];
    if (features.includes(index_js_1.Feature.SMS)) {
        newOrder.push(index_js_1.Feature.SMS);
    }
    if (features.includes(index_js_1.Feature.VOICE)) {
        newOrder.push(index_js_1.Feature.VOICE);
    }
    if (features.includes(index_js_1.Feature.MMS)) {
        newOrder.push(index_js_1.Feature.MMS);
    }
