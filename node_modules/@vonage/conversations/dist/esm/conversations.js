...
const apiToConversation = (response) => {
    ...
    const conversation = Client.transformers.camelCaseObjectKeys(response, true);
...
const conversationToApi = (conversation) => {
    ...
    const apiConversation = Client.transformers.snakeCaseObjectKeys(Client.transformers.omit(['id', 'sequenceNumber', 'timestamp', 'state'], conversation), true);
...
const apiToEvent = (response) => {
    return {
        ...Client.transformers.camelCaseObjectKeys(Client.transformers.omit(['_embedded', '_links'], response), true),
        fromMember: {
            ...Client.transformers.camelCaseObjectKeys(response._embedded?.from_member, true),
        },
        fromUser: {
            ...Client.transformers.camelCaseObjectKeys(response._embedded?.from_user, true),
        },
    };
};
const apiToMember = (response) => {
    return {
        ...Client.transformers.camelCaseObjectKeys(Client.transformers.omit(['_embedded', '_links'], response), true),
        user: {
            ...Client.transformers.camelCaseObjectKeys(Client.transformers.omit(['_links'], response._embedded?.user), true),
        },
    };
};
const memberToApi = (member) => ({
    ...Client.transformers.snakeCaseObjectKeys(Client.transformers.omit([
        'id',
        'timestamp',
        'conversationId',
        'initiator',
...
export class Conversations extends Client {
    ...
    async getConversationPage(filter) {
        ...
        const resp = await this.sendGetRequest(url, Client.transformers.omit(['by_user'], Client.transformers.snakeCaseObjectKeys(filter)));
        return {
    ...
    async getUserSessionsPage(userId, filter) {
        const resp = await this.sendGetRequest(`${this.config.apiHost}/v1/users/${userId}/sessions`, Client.transformers.snakeCaseObjectKeys(filter));
        return {
    ...
    async getMemberPage(conversationId, filter = {}) {
        const resp = await this.sendGetRequest(`${this.config.apiHost}/v1/conversations/${conversationId}/members`, Client.transformers.snakeCaseObjectKeys(filter));
        return {
    ...
    async getEventPage(conversationId, filter) {
        const resp = await this.sendGetRequest(`${this.config.apiHost}/v1/conversations/${conversationId}/events`, Client.transformers.omit(['by_user'], Client.transformers.snakeCaseObjectKeys(filter)));
        return {
            pageSize: resp.data.page_size,
